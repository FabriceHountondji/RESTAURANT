        ...
        <div class="card-body">
            ...
            <form id="signupForm" class="col-md-10 mx-auto" method="post" action="{{ route('roles.update', $role->id) }}">
                ...
                <div class="form-group">
                    ...
                    <div>
                        <input type="text" class="form-control" id="name" name="name" placeholder="Entrer la catégorie" value="{{ old('name') ? old('name') : $role->name }}" required />
                    </div>
                </div>

                <div class="position-relative form-group">
                    <label for="exampleText" class="">Description</label>
                    <textarea name="description" id="exampleText" class="form-control"> {{ old('description') ? old('description') : $role->description }} </textarea>
                </div>
